...
#include "monsteer/runSimTrigger_generated.h"
#include "monsteer/proxyStatusMsg_generated.h"
#include "monsteer/statusRequestMsg_generated.h"
...
public:
          ...
          musicTimestep(0.0001)
    {
        ...
        options.add_options()
            ...
            ( "steering",
              po::bool_switch(&enableSteering)->default_value( true ),
...
public:
    ...
    SteeringHandler( MUSIC::Setup* setup,
        ...
        : _subscriber( brion::URI(uri.getScheme() + "://") ),
          _publisher( uri ),
          _state( monsteer::steering::SimulationPlaybackState::PAUSE),
    ...
    void processRequestedSimTime(MUSIC::Runtime * runtime)
    {
        ...
        const double apptime = runtime->time();
        runtime->tick();
        _requestedSimTime -= runtime->time() - apptime;
    }
...
void _onSimulationTrigger( const zeq::Event& event )
{
    ...
    _state = monsteer::steering::SimulationPlaybackState::ONDEMAND;
...
public:
    ...
    void run()
    {
        ...
        {
            if( !_steeringHandler ||  _steeringHandler->getPlaybackState() ==
                    monsteer::steering::SimulationPlaybackState::PLAY )
                runtime.tick();

            if (_steeringHandler &&
                    _steeringHandler->getPlaybackState() ==
                    monsteer::steering::SimulationPlaybackState::ONDEMAND )
                     _steeringHandler->processRequestedSimTime(&runtime);
